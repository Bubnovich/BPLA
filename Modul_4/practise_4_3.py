...
def read_video(drone: Drone):
    ...
    while not stop_flag:
        ...
        if edges.sum() > 100_000:
            drone.update_detected(True)
        else:
            drone.update_detected(False)

        cv2.imshow('Обнаружение препятствия', gray)
        time.sleep(10)
        if cv2.getWindowProperty('Обнаружение препятствия', cv2.WND_PROP_VISIBLE) < 1:
            stop_flag = True
            break
        time.sleep(0.05)
    cap.release()
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        ...
        concurrent.futures.wait([altimeter_future, control_future, video_future])